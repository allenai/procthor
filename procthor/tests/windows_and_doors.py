...
#%%
from procthor.databases import wall_holes, asset_database
import json
...
windows = [(window, "windows") for window in asset_database["Window"]]
doors = [(door, "doors") for door in asset_database["Doorway"]]
for i, (asset, key) in enumerate(windows + doors):
    ...
    house[key].append(
        {
            "assetId": asset_id,
            "assetOffset": wall_holes[asset_id]["offset"],
            "boundingBox": {
                "min": {
                    "x": 0,
                    "y": 0,
    ...
    house[key].append(
        {
            ...
            "boundingBox": {
                ...
                "max": {
                    "x": start_at_x + wall_holes[asset_id]["max"]["x"],
    ...
    house[key].append(
        {
            ...
            "boundingBox": {
                ...
                "max": {
                    "x": (
                        3 - asset["boundingBox"]["x"] + wall_holes[asset_id]["max"]["x"]
                    ),
                    "y": 0.5 + asset["boundingBox"]["y"],
                    "z": 0,
                },
